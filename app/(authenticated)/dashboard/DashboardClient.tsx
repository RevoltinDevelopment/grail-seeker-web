...
export default function DashboardClient() {
  ...
  const { alerts, isLoading: isLoadingAlerts } = useAlerts({ limit: 2, status: 'active' }, { enableToasts: true })
  const { stats, isLoading: isLoadingStats } = useAlertStats()
  ...
  if (isLoadingSearches || isLoadingAlerts || isLoadingStats) {
    return (
    ...
    <div className="container-custom py-12">
      ...
      <div className="mb-12 grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        >
          ...
          <p className="text-3xl font-bold text-collector-blue">{stats.active}</p>
          <p className="mt-1 text-xs text-slate-500">{stats.total} all time matches</p>
        </Link>
      </div>
            ...
            {searches.slice(0, 6).map((search) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  {search.maxPrice && (
                    <span className="text-slate-600">Max: ${search.maxPrice.toLocaleString()}</span>
                  )}
                </div>
              </div>
            ))}
