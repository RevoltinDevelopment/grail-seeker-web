    ...
    <div className="container-custom py-12">
      ...
      {searches.length === 0 && (
        <div className="rounded-lg border border-slate-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="mb-6 text-slate-600">Create your first grail search to start monitoring.</p>
          <Link
            href="/searches/new"
              ...
              <div className="mb-4">
                <span
                  className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${
                    search.isActive ? 'bg-success-green text-white' : 'bg-slate-300 text-slate-700'
                  }`}
                >
                  {search.isActive ? '✅ Active' : '⏸ Paused'}
                </span>
                {(search.currentAlertCount ?? 0) > 0 && (
                  <Link
